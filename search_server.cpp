...
void SearchServer::RemoveDocument(const execution::parallel_policy&,
  ...
  if (document_ids_.count(document_id) != 0) {
    documents_.erase(document_id);
    document_ids_.erase(document_id);
    for_each(execution::par, word_to_document_freqs_.begin(),
             word_to_document_freqs_.end(), [document_id](auto& document) {
...
tuple<vector<string_view>, DocumentStatus> SearchServer::MatchDocument(
  ...
  auto words_end =
      copy_if(execution::par, query.plus_words.begin(), query.plus_words.end(),
